...
## Known Issues Found in Phase 5.7 Review (QnA Frontend Tab)
- api-client.ts line 1001-1002: `userId` and `assistantId` generated with `Date.now()` — both share the same timestamp if called in the same JS tick. IDs are not guaranteed unique. Use a counter or crypto.randomUUID().
- api-client.ts line 119: `response.body?.getReader()` — skips Backstage's `fetchApi` abstraction for SSE reading. This is unavoidable (fetchApi.fetch returns a standard Response), but means SSE path is not interceptable by Backstage's auth token injection layer if the underlying Response body is consumed directly. Low risk for same-origin backend but worth documenting.
- api-client.ts line 134: `JSON.parse(dataLine.slice(6))` — no try/catch. A malformed SSE frame from the backend (e.g. on network truncation) will throw an unhandled promise rejection inside the while loop, propagating up through askQnAStream. The caller (handleSend) does catch it, so the user sees an error message rather than a crash, but the reader is never cancelled/released. Add reader.cancel() in a finally block.
- api-client.ts lines 144-154: Sources fetched via a separate GET /messages after streaming. If the backend has not yet committed the assistant message when the SSE stream ends (timing gap between res.end() and DB write), this will return an empty sources list. This is a known design limitation — acceptable for v1 but worth a TODO.
- EntityCodeInsightContent.tsx line 1001-1002: `userId`/`assistantId` collision risk (same tick) — same issue as above.
- EntityCodeInsightContent.tsx line 1106: `key={idx}` on SourceCard — index-as-key is fine here since the sources array is stable after streaming completes and never reordered, but a source-specific key (e.g. filePath+startLine) would be safer.
- EntityCodeInsightContent.tsx line 539: `buildFileUrl` constructs GitHub URLs by string interpolation without validating repoUrl format. If repoUrl contains query params or trailing slash, the constructed URL is malformed. Low risk since repoUrl is derived from a trusted annotation, but no sanitization is applied.
- EntityCodeInsightContent.tsx line 1161: `repoId = annotation.replaceAll('/', '-')` — org/repo slug with a hyphen in the org name produces the same repoId as different slugs (e.g. `my-org/repo` and `my/org-repo` both become `my-org-repo`). Known limitation of this ID scheme; acceptable for v1 but document the constraint.
- index.ts: `DiagramSection`, `QnASource`, and `CodeInsightApi` methods `createQnASession`/`askQnAStream` are not re-exported from the package index. Plugin consumers cannot import these types without going through the internal api.ts path. Add `export type { DiagramSection, QnASource }` to index.ts.
